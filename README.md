...
2. Integration with NTP: To ensure accurate and reliable enforcement of time-based rules, I integrated Network Time Protocol (NTP). NTP synchronizes the clocks across all devices, mitigating issues related to local clock discrepancies. By using NTP, the system maintains consistent time settings, preventing users from bypassing ACL restrictions through local time adjustments or router resets. This integration enhances the robustness and reliability of the Time-Based ACL system.
